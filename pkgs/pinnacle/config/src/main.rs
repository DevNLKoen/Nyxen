...
async fn config() {
    ...
    let terminal = "kitty";
    let launcher = "rofi";
    let launcher_drun = ["-show", "drun"];
    let launcher_games = ["-show", "games"];
    ...
    // 'mod_key + d` spawns a launcher
    input::keybind(mod_key, 'd')
        .on_press(move || {
            Command::new(launcher).args(launcher_drun).unique().spawn();
        })
        .group("Process")
        .description("Spawn a drun launcher");

    // 'mod_key + g` spawns a launcher
    input::keybind(mod_key, 'g')
        .on_press(move || {
            Command::new(launcher).args(launcher_games).unique().spawn();
        })
        .group("Process")
        .description("Spawn a games launcher");
